...
class LSAAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #
        # The mask is always created with inference_mode(False) so the
        # cached tensor is guaranteed to be a normal (autograd-compatible)
        # tensor regardless of the surrounding execution context
        # (torch.inference_mode(), torch.no_grad(), or full autograd).
        # This prevents the RuntimeError that occurs when a tensor born
        # inside inference_mode is later used in a gradient-tracked forward
        # pass (e.g. Integrated Gradients), while keeping the cache fully
        # effective — no redundant rebuilds during rollout extraction.
        # ----------------------------------------------------------------
        N = dots.shape[-1]
        if (
            self._mask_N != N
