...
class ModelTrainer:
    """Handles model training — LSA ablation variant."""
    ...
    def create_model(self, model_seed: int) -> nn.Module:
        """Create and initialize a SimpleViT_LSA model using a specific seed."""
        ...
        print(
            f"  [LSA] Model created with seed {model_seed}: {total_params:,} params "
    ...
    def train_cv_fold(
        ...
        print(f"\n{'='*70}")
        print(
            f"[LSA] ITERATION {iter_idx + 1}/{self.config['n_iterations']} | "
        ...
        for epoch in range(start_epoch, self.config["num_epochs"]):
            ...
            lsa_logger.end_validation_epoch(
                epoch=epoch, verbose=(epoch % 10 == 0 or epoch == 0)
            )
        ...
        # persist best-epoch temperatures in fold_results
        ...
        # Augment fold_summary with LSA-specific fields
        fold_summary["lsa_best_epoch_temperatures"] = fold_results.get("lsa_best_epoch_temperatures", [])
        fold_summary["lsa_best_epoch_entropy"]       = fold_results.get("lsa_best_epoch_entropy", [])
    ...
    def train_final_model(
        ...
        print(f"\n{'='*70}")
        print(f"[LSA] TRAINING FINAL MODEL FOR {optimal_epochs} EPOCHS")
        ...
        # LSALogger for temperature tracking during final training
        final_training_history.setdefault("lsa_temperatures", [])
        final_training_history.setdefault("lsa_tau_raw",      [])
        final_training_history.setdefault("lsa_attn_entropy", [])

        lsa_logger = LSALogger(
            model=model,
            fold_results=final_training_history,
            iter_idx=-1,
            fold_idx=-1,
            tau_warn_threshold=3.0,
        )
        ...
        for epoch in range(start_epoch, optimal_epochs):
            ...
            # log temperatures — no val loader, just read τ values
            lsa_logger.start_validation_epoch()
            lsa_logger._remove_entropy_hooks()
            temps = lsa_logger._read_temperatures()
            final_training_history["lsa_temperatures"].append(temps["tau_exp"])
            final_training_history["lsa_tau_raw"].append(temps["tau_raw"])
            final_training_history["lsa_attn_entropy"].append(
